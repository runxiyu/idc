...
async def _privmsg_cmd(
    client: entities.Client, args: dict[str, bytes]
) -> None:  # in the future this function should return the raw line sent to the target client
    if not client.user:
...
async def tls_wrapper(s: trio.SocketStream) -> None:
    try:
        ...
        minilog.caution("Some client has messed-up TLS.  Let's hope they're just a spammer and we're not broken...")
